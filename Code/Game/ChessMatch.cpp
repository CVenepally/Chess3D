...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
ChessMatch::ChessMatch(Game* owningGame, int playerOneID, int playerTwoID)
	:m_owningGame(owningGame)
{
	m_chessBoard = new ChessBoard(this, playerOneID, playerTwoID);
...
void ChessMatch::RenderUIText() const
{
	...
	g_gameFont->AddVertsForTextInBox2D(textVerts, "Press '~' to open DevConsole", textBox, 14.f, Rgba8::CYAN, 0.8f, Vec2(0.005f, 0.985f), SHRINK_TO_FIT);
	g_gameFont->AddVertsForTextInBox2D(textVerts, "--------------------------------", textBox, 14.f, Rgba8::CYAN, 0.8f, Vec2(0.005f, 0.97f), SHRINK_TO_FIT);
	...
	{
		matchState = "YET TO START - ChessBegin [firstPlayer=<playerName>] to begin the match";
		break;
	}
	case MatchState::PLAYER_ONE_TURN:
	{
		matchState = Stringf("Match State: %s's(Player 1) Turn", m_playerOneName.c_str());
		break;
	}
	case MatchState::PLAYER_TWO_TURN:
	{
		matchState = Stringf("Match State: %s's(Player 2) Turn", m_playerTwoName.c_str());
		break;
	}
	case MatchState::PLAYER_ONE_WINS:
	{
		matchState = Stringf("Match State: Game Over. %s (Player 1) wins!", m_playerOneName.c_str());
		break;
	}
	case MatchState::PLAYER_TWO_WINS:
	{
		matchState = Stringf("Match State: Game Over. %s (Player 2) wins!", m_playerTwoName.c_str());
		break;
	}
	case MatchState::DRAW:
	{
		matchState = Stringf("Match State: Game Over. DRAW!");
		break;
	}
	default:
		break;
	...
	g_gameFont->AddVertsForTextInBox2D(textVerts, Stringf("%s | %s", cameraControl.c_str(), matchState.c_str()), textBox, 14.f, Rgba8::YELLOW, 0.8f, Vec2(0.005f, 0.955f), SHRINK_TO_FIT);
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
std::string ChessMatch::GetBoardAsString()
{
	std::string rankStrings[8];
...
void ChessMatch::SwitchTurns()
{
	...
	m_owningGame->SwitchCurrentPlayer();

}

//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
std::string ChessMatch::GetDebugModeString() const
{

	switch(m_owningGame->m_debugInt)
	{
		case 0:		return Stringf("Debug Mode: Lit");
		case 1:		return Stringf("Debug Mode: Diffuse Only");
		case 2:		return Stringf("Debug Mode: Diffuse + Light (Normal Maps and SGE disabled");
		case 3:		return Stringf("Debug Mode: Diffuse + Light + Normal Maps (SGE Disabled)");
		case 4:		return Stringf("Debug Mode: Normal Map Raw (Normal Map Texel)");
		case 5:		return Stringf("Debug Mode: (Temp) SGE Raw (SGE Map Texel)");
		case 6:		return Stringf("Debug Mode: Specular Only");
		case 7:		return Stringf("Debug Mode: (Temp) Gloss Only");
		case 8:		return Stringf("Debug Mode: Emissive Only");
		case 9:		return Stringf("Debug Mode: Pixel Normals (World Transformed)");
		case 10:	return Stringf("Debug Mode: Surface Normals");
		case 11:	return Stringf("Debug Mode: Surface Tangents");
		case 12:	return Stringf("Debug Mode: Surface Bitangents");
		case 13:	return Stringf("Debug Mode: Normals Transformed");
		case 14:	return Stringf("Debug Mode: Tangents Transformed");
		case 15:	return Stringf("Debug Mode: Bitangents Transformed");
		case 16:	return Stringf("Debug Mode: UVs");
		case 17:	return Stringf("Debug Mode: Light Strength");
		case 18:	return Stringf("Debug Mode: Vertex Colors Only");
		case 19:	return Stringf("Debug Mode: ???");
	default:
		return "none";
	}
...
void ChessMatch::ChessMove(std::string from, std::string to, std::string promoteTo)
{
	...
	if(m_canTeleport)
	{
		...
		if(m_chessBoard->MovePiece(fromTile, toTile, m_owningGame->GetCurrentPlayerID(), didKingDie))
		{
			if(didKingDie)
			{
	...
	{
		ChessMoveResult moveResult = m_chessBoard->MovePiece(fromTile, toTile, m_owningGame->GetCurrentPlayerID(), promoteTo);
...
bool ChessMatch::OnChessMove(EventArgs& args)
{
	...
	std::string isRemote = args.GetValue("remote", "false");
	...
	if(fromString != "none" && toString != "none")
	{
		...
		if(isRemote != "true")
		{
			std::string remoteCommand = Stringf("RemoteCmd cmd=ChessMove from=%s to=%s", fromString.c_str(), toString.c_str());
			g_devConsole->Execute(remoteCommand);
		}
	}
	else
	{
